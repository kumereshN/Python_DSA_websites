...
def clean_paragraph(t, pattern = '[^a-zA-Z0-9\.\,\-]'):
    lower_txt = t.lower()
    clean_txt = re.sub(pattern, ' ', lower_txt).split()
    return clean_txt
    

def count_vowels_in_word(w):
    find_vowels = [l for l in w if l in VOWELS]
    return (w, len(find_vowels))
...
def get_word_max_vowels(text):
    ...
    for paragraph in text:
        res_d = dict()
        clean_txt_lst = clean_paragraph(paragraph)
        for word in clean_txt_lst:
            w, vowel_count = count_vowels_in_word(word)
            res_d[w] = vowel_count
        word_with_highest_vowel_count = tuple(max(res_d.items(), key = itemgetter(1)))
        res.append(word_with_highest_vowel_count)
    return res
...
print(get_word_max_vowels(paragraphs))