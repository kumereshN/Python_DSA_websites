...
def characterReplacement(s: str, k: int) -> int:
    ...
    * Time: O(N), Space: O(min(N, C)), C = variety of characters
    """
    d = defaultdict(lambda: 0) # frequency for each character in the current window
    ...
    for end, ch in enumerate(s): # for each right-bound of window
        ...
        if max_count < res-k: # if the window as-is is invalid (has too many minority chars)
            d[s[start]] -= 1 # As the window is contracted, we want to remove the count of the ch as it's no longer part of the window
